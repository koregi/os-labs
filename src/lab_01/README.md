...
```
После этого их объявили, присвоили значения, передали параметры в потоки во время функции создания. Далее функция принимает параметры в потоке (дословно из методички):
```c++
    void * proc1(void *arg) {
